...
### Where ad slots live
`<AdSlot>` instances are in `[...page].astro` (two: in-feed + bottom), `noticia/[...slug].astro` (top + bottom), `categoria/[cat].astro` (one), `tag/[tag].astro` (one), `tema/[tema].astro`, `glosario/[termino].astro`, `herramientas/escalado-analogico.astro` and every page under `guias/`. The slot IDs (`1111111111`, `2222222222`, …) are placeholders — when AdSense issues real slot IDs they need to be substituted. The component renders a dashed placeholder until `PUBLIC_ADSENSE_CLIENT` is set; once set it injects `adsbygoogle` `<ins>` tags.

**The prop is `adSlot`, never `slot`.** `slot` is a reserved Astro directive (it assigns an element to a named slot of the parent). Written as `<AdSlot slot="5555555555" />`, Astro read it as "send this component to the slot named 5555555555", found no such slot, and **silently dropped the whole component** — no error, no warning. That is how the home, the 9 `/categoria/` pages and the 1,563 `/tag/` pages shipped with zero ads rendered while the guides rendered fine. If ads vanish from a page, check this first.

**A page that doesn't deserve the index doesn't deserve ads.** The AdSense review does not look at your Search index — it looks at the pages where ads are served. Serving ads on pages the site itself marks `noindex` is the contradiction that sustains a "low value content" verdict (640 of 687 ad-bearing pages were `noindex`). Pass `indexable={false}` to `<AdSlot>` on any page that emits noindex; `src/lib/indexable.ts` is the source of truth for news. A guard step in `.github/workflows/aggregate.yml` fails the deploy if any `noindex` page ships an `<ins class="adsbygoogle">`.
